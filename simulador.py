...
def construir_mazo_combinado(nombre_jugador):
    ...
    # Mapeo parte→clave de distribución
    EQUIP_KEYS = {
        'ARMA':   'Armas',
        'PECHERA':'Pechera',
        'CASCO':  'Casco',
    ...
    for parte in ["ARMA", "BOTAS", "CASCO", "PECHERA", "GUANTES"]:
        ...
        # Reconstruir disp respetando repeticiones y limpiando espacios
        disp = []
        for nom in noms:
            clave = nom.strip().replace(" ", "").upper()
            # buscamos primera carta con ese código de nomenclatura
            carta_obj = next(
                (c for c in todas_cartas if c.nomenclatura.replace(" ", "").upper() == clave),
                None
            )
            if carta_obj:
                disp.append(carta_obj)
            else:
                print(f"¡Atención! Nomenclatura desconocida: '{nom}' en set {set_name}")

        usados = []
        dist = distribucion_equipamiento[EQUIP_KEYS[parte]]

        # Selección según distribución
        for nivel, cantidad in dist.items():
            candidatos = [c for c in disp if c.nivel == nivel and c not in usados]
            if len(candidatos) < cantidad:
                print(f"Faltan {cantidad - len(candidatos)} cartas de nivel {nivel} en {parte} de {set_name}")
            seleccionadas = candidatos[:cantidad]
            mazo.extend(seleccionadas)
            usados.extend(seleccionadas)

    # 10 cartas adicionales (como siempre)
    print("\n--- Selección de 10 cartas adicionales ---")
    extras = {1:2, 2:2, 3:2, 4:2, 5:2}
    for nivel, cant in extras.items():
        mazo.extend(elegir_cartas_por_nivel(todas_cartas, nivel, cant))

    print(f"\nTotal cartas en {nombre_jugador}: {len(mazo)} (deberían ser 40)")
    return mazo, origen
